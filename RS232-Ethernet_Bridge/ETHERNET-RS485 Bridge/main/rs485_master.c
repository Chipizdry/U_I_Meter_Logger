...
/*
static SemaphoreHandle_t s_ws_mutex = NULL; // mutex для WebSocket

void rs485_ws_init(void)
{
    ...
    while (1) {
        if (xQueueReceive(s_req_queue, &req, portMAX_DELAY) == pdTRUE) {
            ...
            if (rx_len > 0) {
                ...
                char hex_resp[256] = {0};
                for (int i = 0; i < rx_len && i < 128; i++) {
                    char tmp[4];
                    snprintf(tmp, sizeof(tmp), "%02X", resp[i]);
                    strncat(hex_resp, tmp, sizeof(hex_resp) - strlen(hex_resp) - 1);
                }

                char ws_msg[512];
                snprintf(ws_msg, sizeof(ws_msg), "{\"hex_response\":\"%s\"}", hex_resp);

                // Отправка через WebSocket с защитой мьютексом
                if (s_ws_mutex && xSemaphoreTake(s_ws_mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
                    websocket_send_text(ws_msg);
                    xSemaphoreGive(s_ws_mutex);
                    ESP_LOGI(TAG, "Sent response back to WebSocket");
                } else {
                    ESP_LOGW(TAG, "WebSocket busy, cannot send response");
                }
            } else {
                ESP_LOGW(TAG, "No response from RS485 slave");
            }
