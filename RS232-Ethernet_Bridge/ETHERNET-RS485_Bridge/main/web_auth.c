 ...
 esp_err_t login_post_handler(httpd_req_t *req)
{
 ...
 bool check_ota_session(httpd_req_t *req)
    {
        size_t len = httpd_req_get_hdr_value_len(req, "X-OTA-Session") + 1;
        if (len <= 1) return false;

        char *hdr = malloc(len);
        if (!hdr) return false;

        if (httpd_req_get_hdr_value_str(req, "X-OTA-Session", hdr, len) != ESP_OK) {
            free(hdr);
            return false;
        }

        fs_session_t stored_session;
        memset(&stored_session, 0, sizeof(stored_session));

        nvs_handle_t h;
        bool ok = false;
        if (nvs_open("ota", NVS_READONLY, &h) == ESP_OK) {
            size_t sz = sizeof(stored_session);
            esp_err_t err = nvs_get_blob(h, "fs_session", &stored_session, &sz);
            nvs_close(h);

            if (err == ESP_OK) {
                stored_session.token[32] = 0; // нуль-терминатор на всякий случай
                if (strncmp(hdr, stored_session.token, 32) == 0) {
                    ok = true;
                }
            }
        }

        ESP_LOGW(TAG, "FS token from header: %s", hdr);
        ESP_LOGW(TAG, "FS token stored in NVS: %s", stored_session.token);
